...
static bool viewport_opened = false;

bool IsViewportShown()
{
    return viewport_opened;
}

void ShowViewport(GameAssetsBrowser& asset_browser)
{
    viewport_opened = ImGui::Begin("Viewport", nullptr, ImGuiWindowFlags_HorizontalScrollbar);
    if (viewport_opened)
    {
        ...
        ImGui::SetCursorPos({(content_size.x - image_width) * 0.5f, (content_size.y - image_height + pady) * 0.5f});
        ...
        if (!is_game_view_opened || (is_game_view_opened && ImGui::IsWindowHovered()))
        {
            SetInteractingCamera(&editor_cam);
            SetInteractingTexture(&texture);

            active_viewport_content_info.content_size = Vec2f{image_width, image_height};
            active_viewport_content_info.content_pos = Vec2f{pos.x, pos.y};

            float f_x = editor_cam.GetSize().x / image_width;
            float f_y = editor_cam.GetSize().y / image_height;

            Mouse::SetRelativeMousePosition({(int) ((ImGui::GetMousePos().x - pos.x) * f_x), (int) ((ImGui::GetMousePos().y - pos.y) * f_y)});
        }
        ...
        if (ImGui::BeginDragDropTarget())
        {
            if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("dnd tilesheettile"))
            {
                ...
                if (!stex)
                {
                    LOGERRORF("[UI:Viewport Drag Drop tilesheet] texture can't be loaded "
        ...
        {
            // ImGui::GetWindowDrawList()->AddLine( pos + ImVec2{i * grid_size.x, 0},
            // pos + ImVec2{i * grid_size.x, content_size.y}, IM_COL32_WHITE);
        }
            ...
            if (d.x || d.y)
            {
                ...
                if (selected_obj)
                {
                    if (drag_snap)
                    {
                        ...
                        // if (selected_obj_bounds.width != 0 && selected_obj_bounds.height
                        // != 0)
                        //{
                        //	for (const auto& obj : Scene::current_scene.objects_all)
                        //	{
        ...
        if (ImGui::IsItemHovered())
        {
            if (!is_moving_gizmo)
            {
                ...
                {
                    // LOGDEBUGF("{} {} {} {}", ImGui::GetMousePos().x,
                    // ImGui::GetMousePos().y, image_cursor_pos.x, image_cursor_pos.y);
                ...
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
                {
                    ...
                    for (const auto& obj : Scene::current_scene.objects_all)
                    {
                        ...
                        {
                            // at the same mouse position get an object other then the
                            // previous selected objects #TODO: this now only works between
                            // two object and switches between them first refactor
                ...
                if (ImGui::BeginDragDropTarget())
                {
                    ...
                    if (payload != nullptr)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                LOGERRORF("[UI:Tools:open tilemaps] {} is not a valid file for "
                                          "a tilemap",
                                    src);
                                break;
                            }
                            for (const auto& t : opened_tilemaps)
                            {
                                if (t.path == src)
                                {
                                    LOGDEBUGF("[UI:Tools:open tilemaps] {} already is an "
                                              "existing tilemap",
                                        src);
                                    break;
                                }
                ...
                if (ImGui::TreeNode("preview"))
                {
                    ...
                    if (preview_size)
                    {
                        ...
                        for (uint32_t x = 0; x < t.tile_map.getSize().x; x += t.size.x)
                        {
                            ...
                            {
                                draw->AddLine(screen_pos + ImVec2{(float) x, (float) y}, screen_pos + ImVec2{(float) x, (float) t.tile_map.getSize().y}, IM_COL32(255, 255, 255, 60));
                                draw->AddLine(screen_pos + ImVec2{(float) x, (float) y}, screen_pos + ImVec2{(float) t.tile_map.getSize().x, (float) y}, IM_COL32(255, 255, 255, 60));
                            }
                        }
                    }
                    ImGui::TreePop();
